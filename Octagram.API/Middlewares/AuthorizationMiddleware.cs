...
/// </summary>
public class AuthorizationMiddleware(RequestDelegate next, IAuthService authService, IUserRepository userRepository, string[]? roles = null, bool allowAnonymous = false)
{
    private readonly string[] _roles = roles ?? ["User", "Admin"];
...
public async Task InvokeAsync(HttpContext context)
{
    ...
    var rolesClaim = claimsIdentity?.FindFirst(ClaimTypes.Role)?.Value;
    var userName = claimsIdentity?.FindFirst(ClaimTypes.Name)?.Value;

    if (!_roles.Any(role => rolesClaim != null && rolesClaim.Contains(role, StringComparison.OrdinalIgnoreCase)))
    {
